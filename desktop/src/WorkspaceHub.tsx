...
const modeNames: Record<string, string> = { running: '正在下载', pausing: '等待当前条结束', paused: '已暂停', idle: '服务待命',
  waiting_login: '等待登录', logging_in: '正在登录', switching: '回收/切换浏览器中', resource_paused: '资源保护 · 暂缓运行' };
...
const total = (w: WorkspaceInfo) => Object.values(w.state.counts).reduce((a, b) => a + b, 0) - count(w, 'cancelled');
...
export default function WorkspaceHub({ children }: { children: (w: WorkspaceInfo, overview: () => void, manage: () => void, switchTo: (id: string) => void, workspaces: WorkspaceInfo[]) => React.ReactNode }) {
  ...
  const [hub, setHub] = useState<WorkspaceSnapshot>({ version: '6.2.0', limit: 4, workspaces: [], closing: false });
  ...
  const [loginModes, setLoginModes] = useState<Record<string, boolean>>({});
  ...
  const [resources, setResources] = useState<{ available: number | null; capacity: number; shm?: { used: number; size: number } | null } | null>(null);
  ...
  const running = active.filter(w => ['running', 'pausing'].includes(w.state.mode) || w.state.run_requested);
  ...
  useEffect(() => {
    ...
    return api.subscribe(event => {
      ...
      if (event.event === 'resources') setResources(event.data);
  ...
  async function login(ids: string[]) {
    const entries = ids.filter(id => urls[id]?.trim()).map(id => ({ id, url: urls[id].trim(), headless: loginModes[id] ?? active.find(w => w.id === id)?.headless ?? false }));
  ...
  const readyToConfirm = active.some(w => w.online && w.state.session_ready && !w.busy);
  const status = (w: WorkspaceInfo) => w.busy === 'login' ? '登录中' : w.state.resource_hold ? '资源保护 · 暂缓运行' : w.state.resources?.parked ? '会话已暂存 · 等待继续' : w.state.session_ready ? (w.confirmed ? modeNames[w.state.mode] || '已就绪' : '已登录 · 待确认') : w.error ? '需要处理' : '等待登录';
  ...
  return <>
    {current ? children(current, () => select(null), () => setWizard(true), show, active) : <div className="shell hub-shell">
      <aside className="sidebar"><div className="brand"><span className="logo"><ArrowDownToLine size={24}/></span><div><b>TMIS</b><span>数据工作台</span></div><span className="version">6.2</span></div>
      ...
      <section className="workspace"><header className="topbar"><span>工作空间 <ChevronRight size={14}/> 并行总览</span><button className="btn" onClick={() => windowAction('compact')}><Monitor size={16}/>悬浮窗</button></header><main>
        ...
        {resources && <div className={'notice ' + (resources.capacity < 4 ? 'warning' : '')}><ShieldCheck size={18}/><span>稳定运行保护 · 系统可用内存 {resources.available === null ? '未知' : (resources.available / 1024**3).toFixed(1) + ' GB'} · 当前并发上限 {resources.capacity}
          {resources.shm && ' · /dev/shm ' + (resources.shm.used / 1024**2).toFixed(0) + ' / ' + (resources.shm.size / 1024**2).toFixed(0) + ' MB'}</span></div>}
        {pending.length > 0 && <div className="onboarding-card"><span className="step-number">01</span><div><b>{pending.length} 个工作区尚未登录</b><p>按需登录任意一个即可开启，不影响其他队列；也可批量登录。</p></div><button className="btn primary" disabled={hub.closing} onClick={() => setWizard(true)}><Link size={16}/>管理登录</button></div>}
        ...
        {failedOnly ? <section className="panel all-failures">{active.filter(w => failed(w)).map(w => <article key={w.id}>
          <CircleAlert size={18}/><div><b>{w.name}</b><small>{failed(w)} 条失败、超时或中断任务</small><p>进入此工作区的“待处理任务”分页查看与重试。</p></div><button className="btn" onClick={() => show(w.id)}>进入所属队列 <ChevronRight size={15}/></button></article>)}
          ...
          <p className="workspace-current">{w.state.current_task?.output_name || w.state.tasks.find(t => t.id === w.state.current)?.output_name || (w.confirmed ? '在此队列中添加参数表，或继续已有任务' : '登录此工作区即可继续，无需等待其他工作区')}</p>
    ...
    {wizard && <div className="modal-shade"><section className="modal login-wizard" role="dialog" aria-modal="true" aria-labelledby="login-wizard-title">
      <div className="modal-header"><div><div className="eyebrow">INDEPENDENT WORKSPACES</div><h2 id="login-wizard-title">独立登录，按需开启各自队列</h2></div><button className="icon-button" aria-label="关闭登录向导" onClick={closeWizard}><X size={20}/></button></div>
      <div className="modal-body"><div className="notice"><ShieldCheck size={18}/><span>每个链接对应独立浏览器。提交后清空输入；不保存登录 URL 或 Token。已运行的工作区不受新增登录影响。</span></div>
        ...
        <div className="login-rows">{active.map((w, i) => <article className="login-row" key={w.id} data-workspace-id={w.id}>
          ...
          <div className="setting-row"><label>登录模式<select aria-label={'登录模式 ' + w.name} disabled={!!w.busy || !!w.state.current || (w.state.session_ready && !urls[w.id]?.trim())}
            value={(loginModes[w.id] ?? w.headless ?? false) ? 'headless' : 'headed'} onChange={e => setLoginModes(m => ({ ...m, [w.id]: e.target.value === 'headless' }))}>
            <option value="headed">有头 · 显示浏览器</option><option value="headless">无头 · 后台下载（推荐）</option></select></label>
            {w.confirmed && w.state.session_ready && <button className="btn primary" onClick={() => { closeWizard(); show(w.id); }}>进入此队列</button>}</div>
      ...
      </div><div className="modal-footer"><span>{active.filter(w => w.state.session_ready).length} / {active.length} 工作区已登录</span><button className="btn" disabled={!!busy || !pending.some(w => urls[w.id]?.trim())} onClick={() => void login(pending.map(w => w.id))}>全部登录</button>
        <button className="btn primary" disabled={!readyToConfirm || !!busy || hub.closing} onClick={async () => {
          if (!api) return;
          setBusy('confirm');
          try { await api.workspaces('confirm', { ids: active.filter(w => w.online && w.state.session_ready && !w.busy).map(w => w.id) }); await refresh(); closeWizard(); select(null); setToast('已登录的工作区可独立配置和运行，其余队列保持不动。'); }
          catch (error) { setToast(errorText(error)); } finally { setBusy(''); }
        }}><CheckCircle2 size={16}/>进入已登录工作区</button></div>
    </section></div>}
