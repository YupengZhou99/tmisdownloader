...
class WorkspaceManager extends EventEmitter {
    ...
    if (fs.existsSync(this.registryPath)) {
      ...
      for (const item of registry.items) {
        ...
        this.addEntry({ id: item.id, name: safeName(item.name), root: item.root,
          archived: !!item.archived, headless: item.headless === true, folder: this.folder(item.id), browserPath: typeof item.browserPath === 'string' && (!item.browserPath || path.isAbsolute(item.browserPath)) ? item.browserPath : '' });
      }
  ...
  addEntry(meta) { this.entries.set(meta.id, { meta, state: emptyState(), fingerprints: new Set(), logs: [], error: '', confirmed: false, bridge: null, online: false, refreshing: null, timer: null, dirty: false, previews: new Map(), busy: null }); }
  ...
  async ensure(id) {
    ...
    bridge.on('event', message => {
      ...
      if (message.event === 'log') {
        message = { ...message, log_id: id + ':' + (e.logSerial = (e.logSerial || 0) + 1), time: new Date().toLocaleTimeString('zh-CN', { hour12: false }) };
        e.logs = [...e.logs, message].slice(-300);
      }
      ...
      if (message.event === 'offline') {
        ...
        const counts = { ...e.state.counts };
        if (counts.running) { counts.interrupted = (counts.interrupted || 0) + counts.running; delete counts.running; }
        e.state = { ...e.state, tasks, counts, session_ready: false, current: null, current_task: null,
          resources: {}, run_requested: false, mode: 'waiting_login', switching: false, pending_headless: null };
      }
      this.emit('event', { ...message, workspaceId: id });
      if (['ready', 'changed', 'status', 'completed'].includes(message.event)) this.schedule(id);
      if (message.event !== 'log') this.changed();
    });
  ...
  async refresh(id) {
    ...
    e.refreshing = (async () => {
      ...
      state.batches.forEach(b => e.fingerprints.add(b.fingerprint));
  ...
  snapshot() {
    ...
    return { version: '6.2.0', limit: MAX_WORKSPACES, workspaces, closing: this.closing };
  }
  create(name = '新工作区') {
  ...
  update(id, data) {
    const e = this.get(id);
    if (!data || Object.keys(data).some(k => !['name', 'root', 'browserPath', 'headless'].includes(k))) throw new Error('工作区设置无效');
    const next = { ...e.meta };
    if ('name' in data) next.name = safeName(data.name);
    if ('headless' in data) { if (typeof data.headless !== 'boolean') throw new Error('浏览器模式无效'); next.headless = data.headless; }
  ...
  async confirm(ids) {
    if (!Array.isArray(ids) || !ids.length || new Set(ids).size !== ids.length) throw new Error('请选择需要确认的工作区');
    const pending = ids.map(id => this.get(id));
    if (pending.some(e => e.meta.archived)) throw new Error('不能确认已归档的工作区');
    await Promise.all(pending.map(e => this.refresh(e.meta.id)));
    if (pending.some(e => e.meta.archived || !e.online || !e.state.session_ready || e.busy)) throw new Error('所选工作区尚未登录成功');
    pending.forEach(e => { e.confirmed = true; }); this.changed();
  }
  requireConfirmed(e) {
    if (!e.confirmed) throw new Error('请先登录此工作区；无需等待其他工作区');
  }
  async login(id, data) {
    const e = this.get(id);
    ...
    try {
      ...
      const headless = data.headless ?? e.meta.headless ?? false;
      if (typeof headless !== 'boolean') throw new Error('浏览器模式无效');
      const result = await bridge.call('login', { url: data.url, browser_path: data.browser_path ?? e.meta.browserPath, headless });
      await this.refresh(id);
      if (!e.state.session_ready) throw new Error(e.error || '尚未进入 TMIS 工作界面，请检查此登录链接');
      e.confirmed = true;
      e.meta.headless = headless; this.save();
      return result;
    } finally { e.busy = null; this.changed(); }
  }
  async otherDuplicates(id, fingerprints) {
    const matches = await Promise.all([...this.entries.values()].filter(e => e.meta.id !== id).map(async e => {
      if (e.bridge && !e.bridge.closed) return await e.bridge.call('has_fingerprints', { fingerprints }) ? e.meta.name : null;
      // Archived workspaces retain their last summaries; restoring one refreshes its worker.
      return fingerprints.some(f => e.fingerprints.has(f)) ? e.meta.name : null;
    }));
  ...
  async call(id, command, data = {}) {
    ...
    if (['tasks', 'details'].includes(command)) return e.bridge.call(command, data);
    ...
    if (command === 'preview') for (const p of result) if (p.id) {
      const duplicates = await this.otherDuplicates(id, [p.fingerprint]);
    ...
    if (command === 'request_mode') { e.meta.headless = data.headless; this.save(); }
    await this.refresh(id);
    if (command === 'cancel_mode') { e.meta.headless = e.state.headless; this.save(); }
  ...
  async import(id, data) {
    ...
    try {
      ...
      const names = await this.otherDuplicates(id, data.ids.map(key => e.previews.get(key)));
  ...
  async all(command) {
    ...
    const results = await Promise.all(this.active().map(async e => {
      try {
        if (command === 'start' && (!e.confirmed || (!e.state.session_ready && !e.state.resources?.parked))) throw new Error('此工作区尚未登录，已跳过；其他就绪队列正常启动');
  ...
  async archive(id) {
    ...
    try {
      ...
      if (e.bridge && !e.bridge.closed) e.fingerprints = new Set(await e.bridge.call('fingerprints'));
