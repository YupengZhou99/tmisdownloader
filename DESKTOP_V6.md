# TMIS Workbench V6.1 · 四工作区并行版 · 麒麟 x64
...
在 GitHub Actions 的 **Build TMIS Workbench V6.1 Kylin x64** 成功运行中下载 `TMIS-Workbench-V6.1-Kylin-x64` artifact。解开 artifact ZIP，再解开其中的 `.tar.gz`，完整保留目录结构。在图形桌面的终端运行：
...
1. 在“工作区总览”点击“批量登录与确认”，每行粘贴一个完整 URL，再点击“分配到工作区”。也可逐个创建并命名工作区。Linux 不捕获登录 URL。
2. 点击“全部登录”，等待本轮所有会话进入工作界面。可以“查看浏览器”核对；失败项重试或明确“移出本轮”归档。全部成功后点击“确认并配置任务”，此前禁止参数导入和启动。
...
5. 运行中仍可向指定队列追加参数，或添加新的工作区。新登录不影响已经确认并运行的队列；本轮新增会话需全部确认后才能导入。
6. “暂停本队列”和“全部暂停”都等待各自当前条结束。每个工作区独立设置浏览器最小化/还原、有头/无头切换。悬浮窗汇总四路状态，点击某一路回到对应队列。
...
- 一个后台异常退出时，仅该工作区显示不可用，其他队列继续。提供新 URL 后重新登录并确认，原队列恢复。应用重启后所有会话都需要重新登录确认，工作区与队列身份保持不变。
